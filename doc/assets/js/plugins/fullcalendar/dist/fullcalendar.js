...
var defaults = {
	...
	showHeaderLabel: false,  // aks
	showTimeAxisForAgendaWeek : false //aks
...
$.extend(DayGrid.prototype, {
	...
	renderSegHtml: function(seg, disableResizing) {
		...
		if (!event.allDay && seg.isStart && event.grouped == 0) {
		...
		var fcContent = '<div class="fc-content" >' +
			(isRTL ?
...
$.extend(TimeGrid.prototype, {
	...
	slatRowHtml: function() {
		...
		while (slotTime < this.maxTime) {
			...
			axisHtml =
				(view.opt('showTimeAxisForAgendaWeek') ? '<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
			...
			html +=
				(true ? '<tr ' + (!minutes ? '' : 'class="fc-minor"') + '>' +
					(!isRTL ? axisHtml : '') +
...
$.extend(BasicView.prototype, {
	...
	renderHtml: function() {
		return '' +
			'<table>' +
			(this.opt('showHeaderLabel')? '<thead>' +
					'<tr>' +
...
$.extend(AgendaView.prototype, {
				...
				'<tbody>' +
					'<tr>' +
						'<td class="' + this.widgetContentClass + '">' +
							(this.dayGrid ?
								'<div class="fc-day-grid"/>' +
								(this.opt('showTimeAxisForAgendaWeek') ?'<hr class="' + this.widgetHeaderClass + '"/>':'') :
								''
								) +
	...
	headIntroHtml: function() {
		...
		if (this.opt('weekNumbers')) {
			...
			return (this.opt('showHeaderLabel') == false? '<th class="fc-axis ' + this.widgetHeaderClass + '" ' + this.axisStyleAttr() + '></th>' : '');
		}
	},
	...
	// Queried by the DayGrid subcomponent when generating rows. Ordering depends on isRTL.
	dayIntroHtml: function() {
		return '' + (this.opt('showTimeAxisForAgendaWeek') ?
	...
	slotBgIntroHtml: function() {
		return (this.opt('showTimeAxisForAgendaWeek') ? '<td class="fc-axis ' + this.widgetContentClass + '" ' + this.axisStyleAttr() + '></td>' : '');
	},
	...
	introHtml: function() {
		return (this.opt('showTimeAxisForAgendaWeek') ?'<td class="fc-axis" ' + this.axisStyleAttr() + '></td>' : '');
	},
